...
vec3 calcADS(vec3 ambient, vec3 diffuse, vec3 specular, vec3 lightDir, vec3 point, vec3 point_normal, vec3 eyePoint, Material surfaceMaterial) {
  ...
  return ambient * texture(surfaceMaterial.texture_diffuse1, texCoords).xyz + diffuse * texture(surfaceMaterial.texture_diffuse1, texCoords).xyz + specular * texture(surfaceMaterial.texture_specular1, texCoords).xyz;
}